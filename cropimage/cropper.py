...
class Cropper:
    ...
    @staticmethod
    def center_from_saliency(image, save_dir='./'):
        ...
        maximum_y, maximum_x = np.argwhere(saliency_map == saliency_map.max())[0]
    ...
    def crop(self, image_dir_or_data, completeness=False, target_size=(500, 500)):

        # read image
        image = None
        print(f"Input type: {type(image_dir_or_data)}")
        if type(image_dir_or_data) is str:
            image = cv2.imread(image_dir_or_data)
        elif type(image_dir_or_data) is PIL.Image.Image:
            image = cv2.cvtColor(numpy.array(image_dir_or_data), cv2.COLOR_RGB2BGR)
        elif type(image_dir_or_data) is numpy.array:
            image = image_dir_or_data
        height, width, channel = image.shape
